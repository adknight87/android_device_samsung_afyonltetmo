...
LINEEND=" \\"
COUNT=`wc -l ../afyonltetmo/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../afyonltetmo/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../afyonltetmo/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
